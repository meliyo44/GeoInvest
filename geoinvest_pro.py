...
def get_simulated_news(region_filter="All"):
    """Simulated news data — replace with live NewsAPI call."""
    now = datetime.utcnow()
    ...
    news_pool = {
        "Iran": [
            ...
            {"title": "Les négociations nucléaires avec l'Iran s'effondrent à Vienne, nouvelles sanctions US",
             "title_en": "Iran nuclear talks collapse in Vienna, new US sanctions announced",
        ...
        "Ukraine": [
            ...
            {"title": "Zelensky demande 50 milliards USD aide militaire urgente au Congrès américain",
             "title_en": "Zelensky requests 50bn USD urgent military aid from US Congress",
             "source": "WSJ", "sentiment": -0.40, "impact": "MEDIUM", "zone": "Europe",
             "summary_fr": "Budget défense EU en hausse → Rheinmetall, Thales bénéficiaires",
             "summary_en": "EU defense budget increase → Rheinmetall, Thales beneficiaries",
             "asset_impact": ["DEF"], "age_min": 360},
        ],
        "China-Taiwan": [
        ...
        "EU-Economy": [
            {"title": "BCE signale pause dans les baisses de taux, inflation core persistante en zone euro",
             "title_en": "ECB signals pause in rate cuts, persistent core inflation in eurozone",
             "source": "ECB Press", "sentiment": -0.25, "impact": "MEDIUM", "zone": "Europe",
            ...
            {"title": "Allemagne en récession technique au T3, chômage monte à 6.2%",
             "title_en": "Germany in technical recession Q3, unemployment rises to 6.2%",
        ...
        "Energy": [
            {"title": "OPEP+ maintient coupes production malgré pression US, Brent au-dessus de 87$/b",
             "title_en": "OPEC+ maintains production cuts despite US pressure, Brent above $87/b",
    ...
    all_news = []
    for category, items in news_pool.items():
        for item in items:
    ...
    if region_filter not in ["All", "Monde"]:
        ...
        zones = region_map.get(region_filter, [])
        if zones:
            all_news = [n for n in all_news if n.get('zone') in zones or n.get('zone') == 'Global']
    
    all_news.sort(key=lambda x: abs(x['sentiment']), reverse=True)
    return all_news[:8]
...
def get_market_data():
    ...
    seed = int(time.time() / 1800)  # Changes every 30min
    random.seed(seed)
    
    def rand_price(base, vol=0.015):
        return round(base * (1 + random.gauss(0, vol)), 2)
    
    def rand_change(base_change, vol=0.5):
        return round(base_change + random.gauss(0, vol), 2)
    
    return {
        "XAU/USD": {"price": rand_price(2341), "change": rand_change(+0.8), "unit": "$"},
        "BRENT": {"price": rand_price(87.40), "change": rand_change(+1.2), "unit": "$"},
        "CAC40": {"price": rand_price(7823), "change": rand_change(-0.4), "unit": "pts"},
        "GLD ETF": {"price": rand_price(218.5), "change": rand_change(+0.7), "unit": "$"},
        "USO ETF": {"price": rand_price(79.8), "change": rand_change(+1.1), "unit": "$"},
        "ARGT ETF": {"price": rand_price(58.2), "change": rand_change(-1.5), "unit": "$"},
        "EUR/USD": {"price": rand_price(1.0842, 0.003), "change": rand_change(-0.1, 0.2), "unit": ""},
        "VIX": {"price": rand_price(16.8, 0.05), "change": rand_change(+2.1, 1.0), "unit": "pts"},
    }
...
with st.sidebar:
    ...
    <div style="margin-top:20px; padding:12px; background:#141922; border:1px solid #1e2a3a; border-radius:4px; font-size:10px; color:#8892a4;">
        ...
        <div>NewsAPI: <span style="color:#ffd600;">Demo Mode</span></div>
        <div>Alpha Vantage: <span style="color:#ffd600;">Simulated</span></div>
        <div>GDELT: <span style="color:#ffd600;">Demo Mode</span></div>
        <div style="margin-top:8px; color:#4a5568; font-size:9px;">
            Remplacer les clés API dans<br>les variables en haut du fichier
        </div>
    </div>
    """, unsafe_allow_html=True)
...
news_items = get_simulated_news(region_key)
...
with col_news:
    ...
    for i, news in enumerate(news_items[:6]):
        ...
        <div class="news-card">
            ...
            <div class="sentiment-bar-container" style="margin-top:8px;">
...
# ─── ROW 4: PERFORMANCE CHART + PORTFOLIO SIM ─────────────────────────────────
st.markdown("<hr>", unsafe_allow_html=True)
col_chart, col_portfolio = st.columns([1.6, 1])

with col_chart:
    st.markdown("""<div class="section-header">
        <span class="section-title">📈 PERFORMANCE ACTIFS (24H)</span>
        <div class="section-line"></div>
    </div>""", unsafe_allow_html=True)
    st.plotly_chart(build_asset_performance_chart(market_data), use_container_width=True, config={'displayModeBar': False})
